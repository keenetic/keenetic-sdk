...
use strict;
use warnings;
our @EXPORT = qw(%package %srcpackage %category %subdir %preconfig %features clear_packages parse_package_metadata parse_package_manifest_metadata parse_target_metadata get_multiline);
...
sub parse_target_metadata($) {
	...
	while (<FILE>) {
		...
		/^Target-CPPFLAGS:\s*(.+)\s*$/ and $target->{cppflags} = $1;
		/^Linux-Version:\s*(.+)\s*$/ and $target->{version} = $1;
		/^Linux-Release:\s*(.+)\s*$/ and $target->{release} = $1;
		/^Linux-Kernel-Arch:\s*(.+)\s*$/ and $target->{karch} = $1;
		/^Default-Subtarget:\s*(.+)\s*$/ and $target->{def_subtarget} = $1;
		/^Default-Packages:\s*(.+)\s*$/ and $target->{packages} = [ split(/\s+/, $1) ];
		/^Private:/ and $target->{private} = 1;
		...
		/^Target-Profile-Linux-Version:\s*(.+)\s*$/ and $profile->{version} = $1;
...
sub parse_package_metadata($) {
	...
	while (<FILE>) {
		...
		/^Kernel-Config:\s*(.+)\s*$/ and $pkg->{kconfig} = $1;
...
sub parse_package_manifest_metadata($) {
	...
	while (<FILE>) {
		...
		/^Version:\s*(.+)\s*$/ and $pkg->{version} = $1;
		/^Depends:\s*(.+)\s*$/ and $pkg->{depends} = [ split /\s+/, $1 ];
		/^Executable:\s*(.+)\s*$/ and $pkg->{executable} = $1;
		/^Source:\s*(.+)\s*$/ and $pkg->{source} = $1;
		...
		/^MD5Sum:\s*(.*)\s*$/ and $pkg->{md5sum} = $1;
		/^SHA256sum:\s*(.*)\s*$/ and $pkg->{sha256sum} = $1;
		/^SHA512sum:\s*(.*)\s*$/ and $pkg->{sha512sum} = $1;
	}
