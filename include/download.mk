...
gen_sha256sum = $(shell sha256sum $(DL_DIR)/$(1) | awk '{print $$1}')
...
define DownloadMethod/default
	$(SCRIPT_DIR)/download.pl "$(DL_DIR)" "$(or $(DL_FILE),$(FILE))" "$(HASH)" "$(URL_FILE)" $(foreach url,$(URL),"$(url)")
	@if [ "$(DL_FILE)" != "" ]; then mv $(DL_DIR)/$(DL_FILE) $(DL_DIR)/$(FILE); fi \
	$(if $(filter check,$(1)), \
...
# Replace "__disabled_check__" by "check" to enable validation for mirrors.
define wrap_mirror
$(if $(if $(MIRROR),$(filter-out x,$(MIRROR_HASH))),$(SCRIPT_DIR)/download.pl "$(DL_DIR)" "$(FILE)" "$(MIRROR_HASH)" "" || ( $(3) ),$(3)) \
$(if $(filter __disabled_check__,$(1)), \
...
define is_tag
  $(if $(CONFIG_GIT_SHALLOW_CLONE),git ls-remote --exit-code --tags $(1) $(2) >/dev/null,false)
endef

define DownloadMethod/git
	$(call wrap_mirror,$(1),$(2), \
		echo "Checking out files from the git repository ($(URL))..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
		if $(call is_tag,$(URL),$(SOURCE_VERSION)); then \
			git clone $(OPTS) --branch $(SOURCE_VERSION) --depth 1 $(URL) $(SUBDIR); \
		else \
			git clone $(OPTS) $(URL) $(SUBDIR) && \
			git -C $(SUBDIR) checkout $(SOURCE_VERSION); \
		fi && \
		$(if $(GIT_SUBMODULES),git -C $(SUBDIR) submodule update --init --recursive && ) \
		echo "Packing checkout..." && \
...
define Download/default
  ...
  $(if $(PKG_GIT_SUBMODULES),GIT_SUBMODULES:=$(filter 1,$(PKG_GIT_SUBMODULES)))
endef
