...
FIRMWARE_ALIGN_SIZE = $(call qstrip,$(CONFIG_TARGET_FIRMWARE_ALIGN_SIZE))
TLV_BLOCK_SIZE = $(if $(CONFIG_TARGET_SIGN_FIRMWARE),$(FIRMWARE_ALIGN_SIZE),0x0)
...
define MkImage
	mkimage -A mips -O linux -T kernel -C $(1) \
		...
		-d $(2) $(if $(CONFIG_KERNEL_MTD_NDM_DUAL_IMAGE),-u) $(3)
endef

define CheckSize
	if [ "$$$$(($(3) % $(FIRMWARE_ALIGN_SIZE)))" -ne "0" ]; then \
		printf "\n$(1) size (0x%08x) is not a multiple of a MTD block size (0x%08x)!\n\n" "$(3)" "$(FIRMWARE_ALIGN_SIZE)"; \
		false; \
	elif [ ! -f $(2) ]; then \
		printf "\nNo $(2) file found."; \
		false; \
...
define AlignToBlock
	if [ ! -f $(1) ]; then \
		...
		dd \
			...
			bs=$$$${SIZE} \
			count=1 \
			conv=sync $(if $(CONFIG_KERNEL_NDM_PARTITION),,| tr "\000" "\377") > $(1).pad; \
		dd \
			if=$(1) \
			of=$(1).pad \
			bs=$$$$(($(FIRMWARE_ALIGN_SIZE))) \
			conv=notrunc; \
		mv $(1).pad $(1); \
	fi
endef
...
define Image/Build
  ...
  ifneq ($(CONFIG_KERNEL_IMAGE_CMDLINE_HACK),)
	...
	$(call AlignToBlock,$(KERNEL_FILE),$$$$(($(FIRMWARE_ALIGN_SIZE))))
	#
  ...
  ifneq ($(CONFIG_TARGET_SPLIT_FIRMWARE_SIZE),)
	...
	$(call AlignToBlock,$(KDIR)/root.$(1),$$$$(($(FIRMWARE_ALIGN_SIZE))))
	#
