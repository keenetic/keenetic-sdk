...
ifeq ($(CONFIG_CLEAN_IPKG),)
  ...
  ifneq ($(CONFIG_JSON_CYCLONEDX_SBOM),)
	$(SCRIPT_DIR)/metadata.pl imgcyclonedxsbom \
...
ifneq ($(wildcard $(STAGING_DIR_HOST)/bin/ndmfw),)
  ...
  define Build/ndmfw-pad
	$(if $(CONFIG_TARGET_SIGN_FIRMWARE), \
		dd if=/dev/zero bs=$(call size_expand,$(1)) count=1 conv=sync \
			$(if $(CONFIG_KERNEL_NDM_PARTITION),,| tr "\000" "\377") >> $@ \
	)
  endef
