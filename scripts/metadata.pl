...
sub target_config_features(@) {
	...
	while ($_ = shift @_) {
		...
		/^usbexternal$/ and $ret .= "\tselect HAS_EXTERNAL_USB\n";
		/^usbgadget$/ and $ret .= "\tselect USB_GADGET_SUPPORT\n";
		/^usbstorage$/ and $ret .= "\tselect USB_STORAGE_SUPPORT\n";
		/^usbuas$/ and $ret .= "\tselect USB_UAS_SUPPORT\n";
		...
		/^emmc$/ and $ret .= "\tselect HAS_EMMC\n";
		/^emmc_boot$/ and $ret .= "\tselect HAS_EMMC_BOOT\n";
	}
	return $ret;
}
...
sub dump_cyclonedxsbom_json {
	...
	my $component = {
		name => $ENV{VENDOR_OS},
		version => $ENV{BSP_VERSION},
		'bom-ref' => $ENV{VENDOR_OS},
		supplier => {
			name => $ENV{VENDOR},
			contact => [ @contact ],
		},
...
sub gen_image_cyclonedxsbom() {
	...
	foreach my $name (sort {uc($a) cmp uc($b)} keys %image_packages) {
		...
		my @hashes = (
			{ alg => 'SHA-256', content => $pkg->{sha256sum} },
			{ alg => 'SHA-512', content => $pkg->{sha512sum} },
		);
	...
	print dump_cyclonedxsbom_json(\@components, \@dependencies);
}
...
sub gen_package_cyclonedxsbom() {
	...
	foreach my $name (sort {uc($a) cmp uc($b)} keys %mpkgs) {
		...
		my @properties = (
			...
			{ name => 'executable', value => $pkg->{executable} ? 'executable' : 'non-executable' },
		...
		$components{$name} = {
			...
			purl => "pkg:apk/generic/".$name."@".$version.($pkg->{epoch} ? "?epoch=".$pkg->{epoch} : ""),
			@licenses > 0 ? (licenses => [ @licenses ]) : (),
			$pkg->{cpe_id} ? (cpe => $pkg->{cpe_id}.":".$version) : (),
			$type ? (type => $type) : (),
			$version ? (version => $version) : (),
	...
	my %all_depends;
	foreach my $comp (sort {uc($a) cmp uc($b)} keys %components) {
		my $pkg = $mpkgs{$comp};
		...
		foreach my $dep (@{$pkg->{depends}}) {
			...
			next if grep { $_ eq $dep } @depends;
			next unless exists $components{$dep};
	...
	my @orphan_components;
	foreach (keys %components) {
		push @orphan_components, $_ unless exists $all_depends{$_};
	}

	if (@orphan_components > 0) {
		push @dependencies, {
			ref => $ENV{VENDOR_OS},
			dependsOn => [ @orphan_components ],
		}
	}

	my @c = values %components;

	print dump_cyclonedxsbom_json(\@c, \@dependencies);
}
