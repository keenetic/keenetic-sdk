...
BUILDER_WRT := ~/.ndwrt
BUILDER_DIR := $(shell if [ -d $(BUILDER_WRT) ] && [ -x $(BUILDER_WRT) ]; then cd $(BUILDER_WRT); pwd; fi)
BUILDER_KEY := $(if $(BUILDER_DIR),$(BUILDER_DIR)/builder_06072020.key)
BUILDER_CRT := $(if $(BUILDER_DIR),$(BUILDER_DIR)/builder_06072020.crt)
