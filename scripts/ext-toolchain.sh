...
CFLAGS=""
TOOLCHAIN="."
TOOLCHAIN_BIN_DIRS="bin usr/bin usr/local/bin"
...
test_honour_copts() {
	cat <<-EOT | "${CC}" $CFLAGS -fhonour-copts -o /dev/null -x c - 2>/dev/null
		int main(int argc, char **argv)
		{
			return 0;
		}
...
test_feature() {
	...
	case "$feature" in
		...
		honour_copts) test_honour_copts;	return $? ;;
	esac
	...
	local inc
	local sysroot="$("$CC" $CFLAGS -muclibc -print-sysroot 2>/dev/null)"
...
probe_cc() {
	if [ -z "$CC" ]; then
		local bin
		for bin in $TOOLCHAIN_BIN_DIRS; do
			local cmd
...
while [ -n "$1" ]; do
	...
	case "$arg" in
		...
		--bin-dirs)
			TOOLCHAIN_BIN_DIRS="${TOOLCHAIN_BIN_DIRS:+$TOOLCHAIN_BIN_DIRS }$1"; shift
		;;
