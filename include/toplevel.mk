...
.config: ./scripts/config/conf $(if $(CONFIG_HAVE_DOT_CONFIG),,prepare-tmpinfo)
	...
	$(_SINGLE)$(NO_TRACE_MAKE) -r -s -C target/linux presets
...
scripts/config/qconf:
	@$(_SINGLE)$(SUBMAKE) -s -C scripts/config qconf CC="$(HOSTCC)"
...
download: .config FORCE $(if $(wildcard $(TOPDIR)/staging_dir/host/bin/tar)$(SKIP_TAR_INSTALL),,tools/tar/install)
...
%::
	...
	@( \
		...
		if ./scripts/kconfig.pl '>' .config tmp/.config | grep -q CONFIG; then \
			echo "ERROR: your configuration is out of sync. Please run make menuconfig, oldconfig or defconfig!"; \
			false; \
		fi \
	)
	@+$(SUBMAKE) -r $@
...
distclean:
	...
	[ -d scripts/private ] || find package -type d -name 'files-*' | $(XARGS) rm -rf
